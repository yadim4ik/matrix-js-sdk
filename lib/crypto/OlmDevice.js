 ...
 *
 * @param {Boolean} true if the session was updated with new data
 */
...
OlmDevice.prototype.getFirstKnownGroupMessageIndex = function (
    roomId, senderKey, sessionId
) {
    return this._getInboundGroupSession(
        roomId, senderKey, sessionId, function(session) {
            return session.first_known_index();
        }
    );
};


OlmDevice.prototype.getAllInboundGroupKeysForRoom = function(
    roomId
) {
    var sessionStore = this._sessionStore;
    var webstore = sessionStore.store;
    var result = [];
    ...
    for (var i = 0; i < webstore.length; i++) {
        ...
        if (splits[0] !== "session.e2e.inboundgroupsessions") { continue; }
        var senderKey = splits[1];
        var sessionId = splits[2];

        var r = JSON.parse(webstore.getItem(key));
        if (r.room_id !== roomId) {
            // TODO: this is somewhat wasteful. We could do all rooms at the same time,
            // or rearrange the webstorage.
            // We're going to have to rearrange the webstorage anyway, since our keys
            // mix up base64 with "/"-separated fields
            continue;
        }
        ...
        try {
            session.unpickle(this._pickleKey, r.session);
            var index = session.first_known_index();
            result.push({
                senderKey: senderKey,
                sessionId: sessionId,
                sessionKey: session.session_key(index),
