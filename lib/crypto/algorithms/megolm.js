...
function MegolmDecryption(params) {
    ...
    // A record of the key data we are missing.
    // senderKey -> sessionId -> first known index, or null
    this._missingSessionMap = {};
}
...
MegolmDecryption.prototype.decryptEvent = function(event) {
    ...
    try {
        ...
        if (e.message === 'OLM.UNKNOWN_MESSAGE_INDEX') {
            // register our desire for more keys for this session
            var known_index = this._olmDevice.getFirstKnownGroupMessageIndex(
                event.room_id, content.sender_key, content.session_id
            );
            this._requestKeysForEvent(event, known_index);
        }
    ...
    if (res === null) {
        // register our desire for the keys for this session
        this._requestKeysForEvent(event, null);
        return null;
    }
...
MegolmDecryption.prototype._requestKeysForEvent = function(event, firstKnownIndex) {
    var senderKey = event.content.sender_key;
    var sessionId = event.content.session_id;

    var sessionsForSender = this._missingSessionMap[senderKey];
    if (!sessionsForSender) {
        sessionsForSender = this._missingSessionMap[senderKey] = {};
    }

    if (sessionsForSender[sessionId] !== undefined) {
        // already got this session in the list
        return;
    }

    sessionsForSender[sessionId] = firstKnownIndex;
    this._sendKeyRequestEvent();
};
...
MegolmDecryption.prototype.onRoomKeyEvent = function(event) {
    ...
    var r = this._olmDevice.addInboundGroupSession(
    ...
    var sessionsForSender = this._missingSessionMap[senderKey];
    if (!sessionsForSender || sessionsForSender[sessionId] === undefined) {
        return;
    }
    delete sessionsForSender[sessionId];
    this._sendKeyRequestEvent();

    // TODO: we may now be able to decrypt events we previously couldn't. We
    // should either attempt to do so proactively, and tell the app about any
    // successes, or tell the app about the new keys and let it call back down
    // to decrypt any it likes.
};

MegolmDecryption.prototype._sendKeyRequestEvent = function() {
    console.log("Desired sessions now", JSON.stringify(this._missingSessionMap));

    // TODO: we need to aggregate these, and only send them once we've finished
    // handling a batch of events. The easiest way is probably just to delay it.

    // TODO: this could easily end up getting huge. We should prioritise recent
    // sessions: if nobody is sharing keys for those, there's not much hope for
    // older ones.
    var sessions = [];

    utils.mapProperties(this._missingSessionMap, function(senderKey, sessionsForSender) {
        utils.mapProperties(sessionsForSender, function(sessionId, firstKnownIndex) {
            var s = {
                algorithm: olmlib.MEGOLM_ALGORITHM,
            ...
            if (firstKnownIndex !== null) {
                s.first_known_index = firstKnownIndex;
            }
            sessions.push(s);
        });
    });

    this._crypto.sendKeyRequestEvent(this._roomId, sessions);
};


MegolmDecryption.prototype.shareAllKeysWithDevice = function(
    deviceInfo
) {
    var self = this;
    var payload = {
        type: "m.reshare_room_keys",
        content: {},
    };

    var roomId = this._roomId;

    payload.content.keys = utils.map(
        this._olmDevice.getAllInboundGroupKeysForRoom(roomId),
        function(k) {
            return {
                algorithm: olmlib.MEGOLM_ALGORITHM,
                room_id: roomId,
                sender_key: k.senderKey,
                session_id: k.sessionId,
                session_key: k.sessionKey,
                keys_claimed: k.keysClaimed,
            };
        }
    );

    var sharemap = {};
    sharemap[this._userId] = [deviceInfo];

    return olmlib.ensureOlmSessionsForDevices(
        this._olmDevice, this._baseApis, sharemap
    ).then(function(devicemap) {
        var r = devicemap[self._userId][deviceInfo.deviceId];
        if (!r.sessionId) {
            // no session with this device, probably because there
            // were no one-time keys.
            //
            // TODO: tweak ensureOlmSessionsForDevices so that we can find
            // out what went wrong.
            throw new Error(
                "Unable to start Olm session with device " +
                deviceInfo.deviceId
            );
        }
        ...
        var contentMap = {};
        contentMap[self._userId] = {};
        contentMap[self._userId][deviceInfo.deviceId] = encryptedContent;
