...
Crypto.prototype.setRoomEncryption = function(roomId, config) {
    ...
    // make sure we have a decryptor, too.
    this._getRoomDecryptor(roomId, config.algorithm);
};

/**
 * share all of our known keys with another of our devices.
 */
Crypto.prototype.shareAllKeysWithDevice = function(
    deviceId
) {
    var dev = this.getStoredDevice(this._userId, deviceId);
    if (!dev) {
        throw new Error('Unknown device ' + deviceId);
    }

    if (!dev.isVerified()) {
        throw new Error("Can't share keys with unverified device "+ deviceId);
    }

    var decryptors=[];

    // for each encrypted room...
    for (var roomId in this._roomDecryptors) {
        if (!this._roomDecryptors.hasOwnProperty(roomId)) { continue; }

        // and for each algorithm we've seen in that room...
        for (var alg in this._roomDecryptors[roomId]) {
            if (!this._roomDecryptors[roomId].hasOwnProperty(alg)) {
                continue;
            }

            decryptors.push(this._roomDecryptors[roomId][alg]);
        }
    }

    // do each room in turn
    function next(i) {
        if (i >= decryptors.length) {
            return;
        }

        return decryptors[i].shareAllKeysWithDevice(dev).then(
            function() {
                return next(i+1);
            }
        );
...
Crypto.prototype.sendKeyRequestEvent = function(roomId, sessions) {
    var content = {
        device_id: this._deviceId,
        sessions: sessions
    };
    ...
    // TODO: resends
    this._baseApis.sendMessageEventRaw(roomId, "m.room.key_request", content).done();
};
