...
function MatrixClient(opts) {
    ...
    if (CRYPTO_ENABLED && this.sessionStore !== null) {
        ...
        try {
            ...
            var keys = {};
            keys["ed25519:" + this.deviceId] = e2eKeys.ed25519;
            keys["curve25519:" + this.deviceId] = e2eKeys.curve25519;
            var json = anotherJson.stringify({
        ...
        this._http.authedRequestWithPrefix(
            ...
            for (var userId in res.device_keys) {
                if (userId in notStored) {
                    var devices = res.device_keys[userId];
                    // TODO: Do something with the result of verification.
                    _verifySignatures(userId, devices);
...
function _verifySignatures(userId, devices) {
    var verified = {};
    var keys = {};
    var device;
    var deviceId;
    for (deviceId in devices) { if (devices.hasOwnProperty(deviceId)) {
        ...
        keys["ed25519:" + deviceId] = {
            deviceId: deviceId,
            key: device.keys["ed25519:" + deviceId]
        };
    }}
    for (deviceId in devices) { if (devices.hasOwnProperty(deviceId)) {
        device = devices[deviceId];
        var signatures = device.signatures[userId];
        var encoded = _encodeForSignature(device);
        for (var keyId in signatures) { if (signatures.hasOwnProperty(keyId)) {
            if (keyId in keys) {
                var utility = new Olm.Utility();
                try {
                    var key = keys[keyId].key;
                    var signature = signatures[keyId];
                    utility.ed25519_verify(key, encoded, signature);
                    verified[deviceId] = verified[deviceId] || [];
                    verified[deviceId].push(keys[keyId].deviceId);
                } catch (e) {
                    console.log(
                        "Verification failed for " + deviceId + " " + keyId +
                        ": " + e.message
                    );
                } finally {
                    utility.free();
...
function _encodeForSignature(jsonObject) {
    var copy = utils.deepCopy(jsonObject);
    if ('signatures' in copy) {
        delete copy.signatures;
    }
    if ('unsigned' in copy) {
        delete copy.unsigned;
    }
    return anotherJson.stringify(copy);
}
