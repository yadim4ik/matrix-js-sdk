...
function MatrixClient(opts) {
    ...
    if (this.idBaseUrl) {
        var urlObj = url.parse(this.idBaseUrl);
        this.idServerHost = urlObj.host;
    }
...
/**
 * This requests an email token for validating an email address. This request
 * goes via the Home Server.
 *
 * @param {string} email
...
MatrixClient.prototype.requestEmailToken = function(email, clientSecret,
    ...
    if (!this.idServerHost) {
        throw new Error("Must be initialised with a valid idBaseUrl to call requestEmailToken");
    }
    var params = {
        id_server: this.idServerHost,
    ...
    return this._http.requestWithPrefix(
        callback, "POST", "/register/email/requestToken",
        undefined, params, httpApi.PREFIX_V2_ALPHA
    );
};
