...
MatrixClient.prototype.getStoredDevice = function(userId, deviceId) {
    ...
    return this._crypto.getStoredDevice(userId, deviceId);
};
 ...
 */
MatrixClient.prototype.shareAllKeysWithDevice = function(
    deviceId
) {
    if (!this._crypto) {
        throw new Error("End-to-End encryption disabled");
    ...
    return this._crypto.shareAllKeysWithDevice(deviceId);
};
...
function _sendEventHttpRequest(client, event) {
    if (event.isState()) {
        // this can't happen. we are only called for events sent via sendEvent,
        // which only supports message events.
        throw new Error("state event not supported here");
    }

    return client.sendMessageEventRaw(
        event.getRoomId(), event.getWireType(), event.getWireContent(),
        event._txnId
    );
}
