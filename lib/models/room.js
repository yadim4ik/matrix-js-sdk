...
function Room(roomId, opts) {
    ...
    // map from userId to deviceId to list of session requests
    this._e2eKeyRequests = {};
}
...
Room.prototype.addLiveEvents = function(events, duplicateStrategy) {
    ...
    for (i = 0; i < events.length; i++) {
        try {
            ...
            if (events[i].getType() === 'm.room.key_request') {
                this._addKeyRequest(events[i]);
            }
        } catch (e) {
            console.warn("Error processing event", events[i], e);
        }
    }
};
...
Room.prototype._addKeyRequest = function(event) {
    var userId = event.getSender();
    var deviceId = event.getContent().device_id;
    if (!utils.isString(deviceId)) {
        throw new Error("Invalid 'device_id' key");
    }

    var sessions = event.getContent().sessions;
    if (!utils.isArray(sessions)) {
        throw new Error("Invalid 'sessions' key");
    }

    if(!this._e2eKeyRequests[userId]) {
        this._e2eKeyRequests[userId] = {};
    }
    this._e2eKeyRequests[userId][deviceId] = sessions;
};


Room.prototype.getKeyRequests = function(event) {
    return this._e2eKeyRequests;
};
