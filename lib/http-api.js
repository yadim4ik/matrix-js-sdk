 ...
 *
 * @param {function=} opts.tokenRefreshCallback
 *
...
module.exports.MatrixHttpApi.prototype = {
    ...
    uploadContent: function(file, callback) {
        ...
        if (global.XMLHttpRequest) {
            promise = this._wrapWithAuthRefresh(callback, function(accessToken) {
                var defer = q.defer();
                var xhr = new global.XMLHttpRequest();
                upload.xhr = xhr;
                var cb = requestCallback(defer, null, this.opts.onlyData);
                ...
                defer.promise.abort = xhr.abort.bind(xhr);
    ...
    authedRequest: function(callback, method, path, queryParams, data, opts) {
        ...
        var request_promise = this._wrapWithAuthRefresh(callback, function(accessToken) {
            queryParams.access_token = accessToken;
            return self.request(
                undefined, method, path, queryParams, data, opts
            );
        });
    ...
    _wrapWithAuthRefresh: function(callback, func) {
        var aborting = false;
        ...
        var self = this;
        function doRequest() {
            // were we aborted while the refresh was in progress?
            if (aborting) {
                throw new Error("request aborted");
            }
            var prom = func(self._accessToken);
            if (prom.abort) {
                abortRequest = function() { prom.abort(); };
            } else {
        ...
        if (this._tokenRefreshPromise) {
            // there is a tokenRefresh in progress; wait for it to finish, then make the
            // request (and don't bother with a tokenRefresh if it fails)
            request_promise = this._tokenRefreshPromise.then(doRequest);
        } else {
            // no tokenRefresh in progress: make the request, and if it fails, consider
            // a tokenRefresh and redo the request.
            request_promise = doRequest().catch(function(err) {
                return self._tryTokenRefresh(err).then(doRequest);
            });
        }

        request_promise = request_promise.nodeify(callback);

        request_promise.abort = function() {
            aborting = true;
            if (abortRequest) {
                abortRequest();
            }
    ...
    _tryTokenRefresh: function(err) {
        if (this._tokenRefreshPromise) {
            // refresh already in progress
            return this._tokenRefreshPromise;
        }

        if (!this.opts.tokenRefreshCallback) {
            // no callback, cannot refresh. rethrow the error.
        }

        var self = this;
        var r = this.opts.tokenRefreshCallback(err).then(function(accessToken) {
            self._accessToken = accessToken;
        }).finally(function() {
            self._tokenRefreshPromise = null;
        });

        this._tokenRefreshPromise = r;
        return r;
    },
