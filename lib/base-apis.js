...
/**
 * Get the complete state of a room
 *
 * @param {string} roomId
 * @param {module:client.callback} callback Optional.
 * @return {module:client.Promise} Resolves: TODO
...
/**
 * Send a message (ie, non-state) event.
 *
 * Low-level method: doesn't handle resends, encryption or local echo.
 *
 * @param {string} roomId
 ...
 * @return {module:http-api.MatrixError} Rejects: with an error response.
 */
MatrixBaseApis.prototype.sendMessageEventRaw = function(
    roomId, eventType, content, txnId
) {
    var pathParams = {
        $roomId: roomId,
        $eventType: eventType,
    ...
    var path = utils.encodeUri("/rooms/$roomId/send/$eventType/$txnId", pathParams);
    return this._http.authedRequest(
        undefined, "PUT", path, undefined, content
    );
};
