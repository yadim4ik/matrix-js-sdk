...
function MatrixBaseApis(opts) {
    ...
    var httpOpts = {
        ...
        tokenRefreshCallback: this._tryTokenRefresh.bind(this),
    };
...
MatrixBaseApis.prototype.getAccessToken = function() {
    return this._http.getAccessToken();
};
 ...
 */
MatrixBaseApis.prototype.getRefreshToken = function() {
...
/**
 * Callback to perform a token refresh
 *
 * If the error looks like a token expiry, and we have a refresh token,
 * try doing a tokenrefresh. Otherwise, rethrow the error.
 *
 * TODO: this belongs in MatrixClient really.
 *
 * @param {Error} error from the original request
 * @return {promise} which resolves to the new access token
 */
MatrixBaseApis.prototype._tryTokenRefresh = function(err) {
    ...
    if (!this._refreshToken) {
        // no refresh token, cannot refresh. rethrow the error.
        throw err;
    }

    var self = this;
    return this._http.request(
        undefined, "POST", "/tokenrefresh", null, {
            refresh_token: this._refreshToken,
        }
    ).then(function(result) {
        self._refreshToken = result.refresh_token;
        // TODO: raise an event to tell the app about the new tokens
